...
var openDialogUsers []Dialog
func (m *Mux) OpenDialog(message string,handler HandlerFunc,context *Context,userMessage *discordgo.Message){
	m.Route("",message,handler)
	openDialogUsers=append(openDialogUsers,Dialog{ 
			userId:userMessage.Author.ID,
			handler:handler, 
		})
	context.HasOpenDialog=true
}
func (context Context) CloseDialog(){
	context.HasOpenDialog=false
}
...
func (m *Mux) Route(pattern string, desc string, cb HandlerFunc,afterFunctions ...func()) (*Route, error) {
	r := Route{}
	r.Pattern = pattern
	r.Description = desc
	r.Run = cb
...
func (m *Mux) OnMessageCreate(ds *discordgo.Session, mc *discordgo.MessageCreate) {
	...
	if mc.Author.ID == ds.State.User.ID {
		log.Println(" Ignore all messages created by the Bot account itself")
	...
	ctx := &Context{
		...
		HasOpenDialog: slices.ContainsFunc(openDialogUsers, func(userDialog Dialog) bool {
			return userDialog.userId == mc.Author.ID
		}),
	}
	...
	if(ctx.HasOpenDialog){
		for _,userDialog := range openDialogUsers{
			if(userDialog.userId==mc.Author.ID){
				userDialog.handler(ds, mc.Message, ctx)
			}
		}
		return
